...
#Stopping criterion: standard deviation
##########################################################################

NM_PSO <-function(objective, bound = c(-100,100), dimension, tolerance = 10^(-6),
                  population_multiple = 1, verbose = T){
  ...
  Population_num_NM <- population_multiple * (dimension + 1)
  ...
  Population_num_PSO <- population_multiple * (2 * dimension )
  ...
  while(trigger){
    ...
    #Stopping criterion
    criterion <- apply(Sorted_population[1:Population_num_NM,],1,objective)
    Sc <- sd(criterion)
    if(Sc <= tolerance){trigger <- F}
    if(iteration == 1000 * dimension){trigger <- F}
  ...
  return(list(optimal_point = global_point, optimal_value = evaluation_final, iteration = iteration))
}
...
dimension=30
NM_PSO(f3, bound = c(-10,10), dimension, tolerance = 10^(-4),
       population_multiple = 1, verbose = T)
